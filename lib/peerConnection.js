...
export class PeerConnection {
    ...
    constructor() {
        ...
        this.peerId = "ballie-tv2";
        this.toConnectPeerId = "ballie-tv1";
        this.#createWebRTC();
    ...
    #createWebRTC() {
        ...
        PeerConnection.peerInstance = peer;
        console.log("Peer Instnace : " + PeerConnection.#peerInstance);
    }
    ...
    createClientPeerConnection(peerid) {
        ...
        if (PeerConnection.isConnectedToPeerServer) {
            ...
            if (!this.#connectedPeers[requestedPeer]) {
                // Create 2 connections, one labelled chat and another labelled file.
                var conn = this.#peerInstance.connect(requestedPeer, {
                    label: "chat",
                    serialization: "raw",
                    metadata: { message: "hi i want to chat with you!" },
                });

                conn.on("open", function (con) {
...
function handleConnection() {
    ...
    if (c.label === "chat") {
        ...
        c.on("close", function () {
            delete singletoPeerConnection.#connectedPeers[c.peer];
            console.log("Close Event : delete client");
        });

        singletoPeerConnection.#connectedPeers[c.peer] = c;
    } else {
        console.log(c.label);
    }
}
